...
- 快速添加支持从中文自然语言中提取日期和时间，例如“明天下午3点开会”会保存为标题“开会”、截止时间为明天 15:00。
...
快速添加在点击“添加”或按回车时执行一次纯规则解析，支持“今天/明天/后天”、周/星期/礼拜及对应的“下周”表达，也支持“明早”“今晚”“明晚”等同时携带日期与时段的合成词。时间可使用上午/早上/清早、下午、晚上/傍晚，小时支持阿拉伯数字和中文一至十一（含“两点”），分钟支持“点半”“点一刻”“点三刻”“H点X分/H点XX分”、`15:00` 与 `15点30`。例如“下周一上午9点30分项目会”会得到标题“项目会”和下周一 09:30。

解析器先把注入的当前时间转换到设备本地时区；上午类时段保留小时数，下午增加 12 小时，晚上类时段在 6–11 点范围内增加 12 小时。裸“中午”按 12:00 处理，“中午12点/十二点”可继续组合口语分钟；裸“十二点”仍因午间/午夜歧义而拒绝。只写日期时默认当天 23:59；只写时间且今天该时间已过时取次日同一时间。单独的“明早/今晚/明晚”、全中文分钟（如“九点三十分”）、中文 24 小时制（如“二十点”）以及重复、具体日期、N 天后、英文和冷门时段均不解析，整句按普通标题保存。

手动日期选择器的结果优先于自然语言时间，但已识别的时间词仍会从标题中移除。自然语言得到的本地截止时间进入任务模型前转换为 UTC 保存，读取存档并验证本地语义时需要再调用 `toLocal()`；例如新加坡本地 15:00 会保存为 07:00Z。未来截止时间在支持提醒的平台上沿用现有权限申请与默认提醒流程；明确写“今天”得到的已过期时间会保留，并关闭提醒。
...
- 旧 JSON 缺少 ID 时自动补全
- 旧字符串数组迁移和损坏记录容错
- 快速添加的相对日期、周/星期/礼拜同义词、合成日期词、中文小时、口语分钟、中午规则、标题清理、保守失败和提醒接入
- “下午/晚上”转换为 24 小时制的 `+12` 规则，以及 UTC 存档转回本地时间后与解析结果一致
...
lib/utils/natural_language_task_parser.dart  快速添加中文日期时间规则解析
...
test/natural_language_task_parser_test.dart  自然语言日期解析规则与边界测试
